...
export default {
  async fetch(request: Request, env: Env) {
    ...
    if (request.method === 'GET' && url.pathname.startsWith('/api/content/')) {
      return this.handleContentRequest(request, env);
    }
  ...
  // 处理静态内容请求
  async handleContentRequest(request: Request, env: Env) {
    try {
      const url = new URL(request.url);
      const key = url.pathname.replace('/api/content/', '');
      
      const value = await env.PYTHON_AI_TRAINING_KV.get(key);
  ...
  // 处理AI请求
  async handleAIRequest(request: Request, env: Env) {
    try {
      // 接收前端请求数据（用户消息）
...
interface Env {
  ...
  AI_API_KEY: string;
  PYTHON_AI_TRAINING_KV: KVNamespace;
}
